...
class tree
{
  ...
  public: 
   static int changed;
   tree( const string& f ) 
      : pntr( new trnode( f, { }, (size_t)1 ))
   { }

   tree( const string& f, const std::vector< tree > & subtrees )   
      : pntr( new trnode( f, subtrees, (size_t)1 ))
   { } 
 
   tree( const string& f, std::vector< tree > && subtrees )
      : pntr( new trnode( f, std::move( subtrees ), (size_t)1 ))
   { }
...
public: 
   ...
   void print_all_addreses(){
		std::cout<<getaddress()<<" -> " << this->functor() << " ";
		for(size_t i = 0; i < nrsubtrees(); i++){
				pntr->subtrees[i].print_all_addreses();
   		}
   }
